...
#else
#warning TODO: forcing 16-bit i/o bus
        /* this appears to work around some kind of errata
           where the at91 chip hangs if the i/o bus width
           is switched.  the problem manifests itself on the
           second insertion of a CF card, when the bus is set
           to the default 8-bit mode and then switched to 16 bits
           after the card is probed.  needs additional research.
           solution suggested by Steven Scholz:
           http://lists.arm.linux.org.uk/pipermail/linux-arm-kernel/2005-August/030790.html */
        pr_debug("%s: forcing 16-bit i/o bus\n", driver_name);
        csr |= AT91C_SMC2_DBW_16;

#endif
        /* make sure we don't write nonzeros to reserved bits */
	AT91_SYS->EBI_SMC2_CSR[4] = (csr & 0x7703ffff);
...
#define SM_NWS		(32)		/* wait states; 0=1.5, 1=2.5, ... */
	...
	if (board->vcc_pin)
                at91_set_gpio_direction(board->vcc_pin, 0, 0);

        if (board->rst_pin)
                at91_set_gpio_direction(board->rst_pin, 0, 0);

	/* must be a GPIO; ergo must trigger on both edges */
        at91_set_gpio_direction(board->det_pin, 1, 1);
        at91_set_deglitch(board->det_pin, 1);
	 ...
	 */
	if (board->irq_pin) {
                at91_set_gpio_direction(board->irq_pin, 1, 1);
                at91_set_deglitch(board->irq_pin, 1);
