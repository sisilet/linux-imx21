...
static int pxa2xx_pcmcia_set_mcio( int sock, int speed, int clock )
{
	...
	MCIO(sock) = ((pxa2xx_mcxx_setup(speed, clock)
		...
		& MCXX_HOLD_MASK) << MCXX_HOLD_SHIFT);
#else
#warning TODO: learn to calculate this right
        /* TODO: the problem here is that the first call uses default
           timing settings, which are too fast for the csb625.  what's
           worse, the call to csb625_...hw_init doesn't happen until
           *after* the call to set_timing, which I think is wrong but
           that's the way it is--- and it prevents the csb625 platform
           glue from telling us about our need for slower timing until
           after it's too late.  Oh, and it looks like those timing
           values get lost somewhere along the way and reverted back
           to their defaults anyway.  Plenty to look at.  b.g. 12Aug2005 */
        MCIO(sock) = 0xfcfff;
#endif
	return 0;
}
