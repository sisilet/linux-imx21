...
static ssize_t show_in(struct device *dev, char *buf)
{
    u8 c;
	struct i2c_client *client = to_i2c_client(dev);
...
static ssize_t show_out(struct device *dev, char *buf)
{
    u8 c;
	struct i2c_client *client = to_i2c_client(dev);
...
static ssize_t set_out(struct device *dev, const char *buf,
			 size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct pcf8574_data *data = i2c_get_clientdata(client);
...
static ssize_t show_config(struct device *dev, char *buf)
{
    u8 c;
	struct i2c_client *client = to_i2c_client(dev);
...
static ssize_t set_config(struct device *dev, const char *buf,
			 size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct pcf8574_data *data = i2c_get_clientdata(client);
