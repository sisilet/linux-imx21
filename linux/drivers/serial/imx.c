...
//#define USE_TTYSMX 1  // define to rename serial ports
#if USE_TTYSMX
...
static struct csb536pc_8250_port csb536pc_ports[] = {
    {
        .serial_req = {
            .port = 0,
            .irq = CSB536_UART2_IRQ,
            .flags = UPF_SKIP_TEST | UPF_BOOT_AUTOCONF,
            .xmit_fifo_size = 8,
            .baud_base = 1000000,
            .io_type = SERIAL_IO_MEM,
            .iomem_base = (void *)CSB536_UART2_VIRT,
            .iomem_reg_shift = 1,
            .iomap_base = CSB536_UART2_PHYS,
        }
    }, {
        .serial_req = {
            .port = 0,
            .irq = CSB536_UART3_IRQ,
            .flags = UPF_SKIP_TEST | UPF_BOOT_AUTOCONF,
            .xmit_fifo_size = 8,
            .baud_base = 1000000,
            .io_type = SERIAL_IO_MEM,
            .iomem_base = (void *)CSB536_UART3_VIRT,
            .iomem_reg_shift = 1,
            .iomap_base = CSB536_UART3_PHYS,
        }
    },
};
...
#endif
	.minor          = MINOR_START,
//	.nr             = ARRAY_SIZE(imx_ports) + ARRAY_SIZE(csb536pc_ports),
...
//        if (dev->id < ARRAY_SIZE(imx_ports)) {
            imx_ports[dev->id].port.dev = _dev;
            uart_add_one_port(&imx_reg, &imx_ports[dev->id].port);
            dev_set_drvdata(_dev, &imx_ports[dev->id]);
//        } else {
//            int idx = dev->id - ARRAY_SIZE(imx_ports);
//            int line;
//            line = register_serial(&csb536pc_ports[idx].serial_req);
//            if (line < 0) {
//                printk("Error registering external UART port %d\n", idx);
//                return -ENOMEM;
//            } else {
//                csb536pc_ports[idx].line = line;
//            }
//        }
...
//            int idx = dev->id - ARRAY_SIZE(imx_ports);
//
//            unregister_serial(csb536pc_ports[idx].line);
//        }
