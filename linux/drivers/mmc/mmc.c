...
static void mmc_read_scrs(struct mmc_host *host)
{
	...
	list_for_each_entry(card, &host->cards, node) {
		...
		/* FIXME:
		 * The SD_APP_SEND_SCR semantic seems to be broken on i.MX SDHC controller.
		 * Documentation states in the under line notice, that SDHC cannot support
		 * different block sizes than 512 bytes. Even FreeScale mx21_to2_rel_beta_1.1
		 * has code related to SCR commented out.
		 * Until somebody with signed FreeScale NDA does not provide "confidential"
		 * company information to prove reverse, we consider SDHC as broken for
		 * SD cards and try to support at least some cards by hack bellow.
		 */
		card->scr.sda_vsn = 0;
		card->scr.bus_widths = SD_SCR_BUS_WIDTH_4 | SD_SCR_BUS_WIDTH_1;
		continue;

	    #endif /*CONFIG_MMC_IMX*/
	    ...
	    #ifndef CONFIG_MMC_IMX
		sg_init_one(&sg, (u8*)card->raw_scr, 64);
	    #else /*CONFIG_MMC_IMX*/
		/* The MX1 requires to wait for DMA to finish
		 * and imxmmc does not overcome above lies about size
		 */
